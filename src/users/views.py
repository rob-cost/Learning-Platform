    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if not Topic.objects.filter(subject=profile.chosen_subject).exists():
                app.send_task(
                    "lessons.ai_topic_generator.generate_topic", args=[chosen_subject]
                )
...
def difficulty_assessment_view(request):
    ...
    questions_exists = DifficultyQuestions.objects.filter(
        subject=chosen_subject
    ).exists()

    if not questions_data["success"]:
        messages.error(
            request, "Error in generating assessment. Please try again later."
        )
        return redirect("login")
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if is_retake:
                ...
                messages.info(
                    request,
                    f"Assessment completed. You scored: {assessment_score}. You remain at {previous_level} level.",
                )
            ...
            messages.success(
                request, "Well done you have completed the registration process!"
            )
            return redirect("profile_page")
    else:
    ...
    return render(
        request,
        "difficulty_assessment.html",
        {"form": form, "questions_exists": questions_exists},
    )
...
def profile_view(request):
    ...
    if (
        Topic.objects.filter(
            subject=chosen_subject, difficulty_level=difficulty_level
        ).count()
        != 10
    ):
        app.send_task(
    ...
    for topic in topics:
        ...
        completed_count = UserProgress.objects.filter(
            user=request.user, lesson__in=lessons_topic, completed=True
        ).count()
...
def delete_profile_view(request):
    if request.method == "POST":
        try:
            ...
            messages.success(request, "Profile successfuly  deleted")
...
def retake_assessment_view(request):
    ...
    if progress_percentage > 79:
        ...
        messages.info(request, "Complete at least 80% of the program")
        return redirect("profile_page")
...
def level_choice_view(request):
    ...
    if request.method == "POST":
        ...
        if choice == "update":
            ...
            messages.success(
                request, f"Your level has been updated to {new_level.capitalize()}!"
            )
        else:
            messages.info(request, "You kept your previous level")
...
def next_topic_view(request):
    ...
    if request.method == "POST":
        if current_difficulty_level == "advanced":
            messages.info(
                request,
                f"Well done {request.user.username}. You have successfully completed the course!",
            )
            ...
            profile.save()
            messages.info(
                request, f"Well done {request.user.username}! You advanced level"
            )
...
def topic_lesson_status(request):
    profile = request.user.learningprofile
    topics = Topic.objects.filter(
        subject=profile.chosen_subject, difficulty_level=profile.difficulty_level
    )

    topics_with_lessons = []
    for topic in topics:
        topics_with_lessons.append({"id": topic.id, "topic_status": topic.status})

    return JsonResponse({"topics": topics_with_lessons})
